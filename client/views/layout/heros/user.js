Template.heroUser.onCreated(function() {
  let instance = this;
  let user_id = FlowRouter.getParam('user_id')

  instance.subscribe('user-profile', user_id);

  Tracker.autorun(function() {
    console.log("Is myPost ready?:", FlowRouter.subsReady("user-profile"));
    instance.user = new ReactiveVar(Meteor.users.findOne({_id: user_id}))
  });

  // Particle JS
  initParticlesJS()
})

Template.heroUser.helpers({
  getUser() {
    console.log("get User ", Template.instance().user.get())
    return Template.instance().user.get()
  },
  getTitle() {
    let user = Template.instance().user.get()
    return getDisplayName(user) + "'s profile"
  },
})

let initParticlesJS = function() {
  // Html not loaded, wait 100ms
  if (!$('#particles-js').length) {
    return Meteor.setTimeout(initParticlesJS, 100)
  }
  particlesJS('particles-js', particles_json)
}

let particles_json = {
  "particles": {
    "number": {
      "value": 200,
      "density": {
        "enable": true,
        "value_area": 800
      }
    },
    "color": {
      "value": "#ffffff"
    },
    "shape": {
      "type": "circle",
      "stroke": {
        "width": 0,
        "color": "#000000"
      },
      "polygon": {
        "nb_sides": 5
      },
    },
    "opacity": {
      "value": 0.5,
      "random": false,
      "anim": {
        "enable": false,
        "speed": 1,
        "opacity_min": 0.1,
        "sync": false
      }
    },
    "size": {
      "value": 3,
      "random": true,
      "anim": {
        "enable": false,
        "speed": 40,
        "size_min": 0.1,
        "sync": false
      }
    },
    "line_linked": {
      "enable": true,
      "distance": 150,
      "color": "#ffffff",
      "opacity": 0.4,
      "width": 1
    },
    "move": {
      "enable": true,
      "speed": 4,
      "direction": "none",
      "random": false,
      "straight": false,
      "out_mode": "out",
      "bounce": false,
      "attract": {
        "enable": false,
        "rotateX": 600,
        "rotateY": 1200
      }
    }
  },
  "interactivity": {
    "detect_on": "canvas",
    "events": {
      "onhover": {
        "enable": true,
        "mode": "repulse"
      },
      "onclick": {
        "enable": false,
        "mode": "push"
      },
      "resize": true
    },
    "dispatched_events_selector": '.hero-content, .hero-content .avatar-wrapper',
    "modes": {
      "grab": {
        "distance": 400,
        "line_linked": {
          "opacity": 1
        }
      },
      "bubble": {
        "distance": 400,
        "size": 40,
        "duration": 2,
        "opacity": 8,
        "speed": 3
      },
      "repulse": {
        "distance": 100,
        "duration": 0.4
      },
      "push": {
        "particles_nb": 4
      },
      "remove": {
        "particles_nb": 2
      }
    }
  },
  "retina_detect": true
}
